<<<<<<< HEAD
"""
Compare Cavitation Probability across Different Signal Types

This script loads the simulation data from different signal types (single frequency,
sweep frequency, dual frequency, dual sweep frequency), calculates the cavitation
probability for each, and creates visualizations showing the differences.
"""
...
def compare_cavitation_probability(focal_depth_mm, medium_type='bone', roi_start=10, roi_end=None,
    ...
    --------
    dict
        Dictionary containing cavitation probability values for each signal type
    """
    ...
    for signal_type, file_path in file_paths.items():
        if os.path.exists(file_path):
            ...
            # Calculate MI and cavitation probability
            # Use 180 kHz as the base frequency for all signal types
            frequency = 180e3
            mi = calculate_mechanical_index(pressure, frequency)
            cavitation_prob = calculate_cavitation_probability(mi, frequency)

            # Store data
            data[signal_type] = {
                'pressure': pressure,
                'mi': mi,
                'cavitation_prob': cavitation_prob,
                'x': x,
                'y': y,
                'z': z,
                'metadata': metadata
    ...
    if len(data) >= 2:
        ...
        for type1, type2, title in pairs:
            if type1 in data and type2 in data:
                create_cavitation_comparison_plot(data[type1], data[type2], type1, type2, title, focal_depth_mm, medium_type, output_dir,
    ...
    if len(data) >= 2:
        create_combined_cavitation_plot(data, focal_depth_mm, medium_type, output_dir,
                                     roi_start=roi_start, roi_end=roi_end, exclude_interface=exclude_interface, interface_depth=interface_depth)

    return data


def create_cavitation_comparison_plot(data1, data2, type1, type2, title, focal_depth_mm, medium_type, output_dir,
                              roi_start=10, roi_end=None, exclude_interface=False, interface_depth=5):
    """
    Create comparison plot for Cavitation Probability between two signal types
    ...
    """
    # Get data
    cavitation1 = data1['cavitation_prob']
    cavitation2 = data2['cavitation_prob']
    x = data1['x']
    y = data1['y']
    z = data1['z']

    # Check if dimensions match
    if cavitation1.shape != cavitation2.shape:
        print(f"Error: Cavitation field dimensions don't match: {cavitation1.shape} vs {cavitation2.shape}")
        return
    ...
    difference = cavitation2 - cavitation1
    ...
    # Create mask for ROI
    mask = np.zeros_like(cavitation1, dtype=bool)
    ...
    roi_cavitation1 = cavitation1[mask]
    ...
    relative_diff = np.max(roi_abs_diff) / np.max(roi_cavitation1) * 100 if np.max(roi_cavitation1) > 0 else 0

    print(f"ROI: z = {roi_start} mm to {roi_end} mm")
    if exclude_interface:
        print(f"Excluding interface region: z = 0 mm to {interface_depth} mm")
    print(f"Maximum absolute cavitation probability difference in ROI: {max_diff:.4f}")
    print(f"Mean absolute cavitation probability difference in ROI: {mean_diff:.4f}")
    print(f"Standard deviation of cavitation probability difference in ROI: {std_diff:.4f}")
    print(f"Maximum relative cavitation probability difference in ROI: {relative_diff:.2f}%")

    # Get the middle indices for each dimension
    nx, ny, nz = cavitation1.shape
    mid_x = nx // 2
    mid_y = ny // 2
    ...
    im1 = axes[0, 0].imshow(cavitation1[mid_x, :, :].T,
                           extent=[y.min(), y.max(), z.min(), z.max()],
                           origin='lower', cmap='viridis', vmin=0, vmax=1.0)
    axes[0, 0].set_title(f'{type1.replace("_", " ").title()} Cavitation X-Slice')
    axes[0, 0].set_xlabel('Y (mm)')
    axes[0, 0].set_ylabel('Z (mm)')
    plt.colorbar(im1, ax=axes[0, 0], label='Cavitation Probability')

    # Plot second signal type X-slice (YZ plane at center of x)
    im2 = axes[0, 1].imshow(cavitation2[mid_x, :, :].T,
                           extent=[y.min(), y.max(), z.min(), z.max()],
                           origin='lower', cmap='viridis', vmin=0, vmax=1.0)
    axes[0, 1].set_title(f'{type2.replace("_", " ").title()} Cavitation X-Slice')
    axes[0, 1].set_xlabel('Y (mm)')
    axes[0, 1].set_ylabel('Z (mm)')
    plt.colorbar(im2, ax=axes[0, 1], label='Cavitation Probability')
    ...
    plt.colorbar(im3, ax=axes[0, 2], label='Cavitation Probability Difference')

    # Plot first signal type Y-slice (XZ plane at center of y)
    im4 = axes[1, 0].imshow(cavitation1[:, mid_y, :].T,
                           extent=[x.min(), x.max(), z.min(), z.max()],
                           origin='lower', cmap='viridis', vmin=0, vmax=1.0)
    axes[1, 0].set_title(f'{type1.replace("_", " ").title()} Cavitation Y-Slice')
    axes[1, 0].set_xlabel('X (mm)')
    axes[1, 0].set_ylabel('Z (mm)')
    plt.colorbar(im4, ax=axes[1, 0], label='Cavitation Probability')

    # Plot second signal type Y-slice (XZ plane at center of y)
    im5 = axes[1, 1].imshow(cavitation2[:, mid_y, :].T,
                           extent=[x.min(), x.max(), z.min(), z.max()],
                           origin='lower', cmap='viridis', vmin=0, vmax=1.0)
    axes[1, 1].set_title(f'{type2.replace("_", " ").title()} Cavitation Y-Slice')
    axes[1, 1].set_xlabel('X (mm)')
    axes[1, 1].set_ylabel('Z (mm)')
    plt.colorbar(im5, ax=axes[1, 1], label='Cavitation Probability')
    ...
    plt.colorbar(im6, ax=axes[1, 2], label='Cavitation Probability Difference')
    ...
    # Add title
    plt.suptitle(f'Cavitation Probability Comparison: {title} ({medium_type.capitalize()}, {focal_depth_mm} mm focal depth)', fontsize=16)

    plt.tight_layout(rect=[0, 0.03, 1, 0.97])

    # Save figure
    output_file = f'{output_dir}/cavitation_comparison_{type1}_vs_{type2}_{medium_type}_{focal_depth_mm}mm.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"Saved cavitation comparison plot to {output_file}")
    ...
    im1 = axes[0].imshow(cavitation1[:, :, focal_z_idx],
                        extent=[x.min(), x.max(), y.min(), y.max()],
                        origin='lower', cmap='viridis', vmin=0, vmax=1.0)
    axes[0].set_title(f'{type1.replace("_", " ").title()} Cavitation Z-Slice at Z={z[focal_z_idx]:.2f} mm')
    axes[0].set_xlabel('X (mm)')
    axes[0].set_ylabel('Y (mm)')
    plt.colorbar(im1, ax=axes[0], label='Cavitation Probability')

    # Plot second signal type Z-slice (XY plane at focal depth)
    im2 = axes[1].imshow(cavitation2[:, :, focal_z_idx],
                        extent=[x.min(), x.max(), y.min(), y.max()],
                        origin='lower', cmap='viridis', vmin=0, vmax=1.0)
    axes[1].set_title(f'{type2.replace("_", " ").title()} Cavitation Z-Slice at Z={z[focal_z_idx]:.2f} mm')
    axes[1].set_xlabel('X (mm)')
    axes[1].set_ylabel('Y (mm)')
    plt.colorbar(im2, ax=axes[1], label='Cavitation Probability')
    ...
    plt.colorbar(im3, ax=axes[2], label='Cavitation Probability Difference')
    ...
    # Add title
    plt.suptitle(f'Cavitation Probability Comparison at Focal Plane: {title} ({medium_type.capitalize()}, {focal_depth_mm} mm focal depth)', fontsize=16)

    plt.tight_layout(rect=[0, 0.03, 1, 0.97])

    # Save figure
    output_file = f'{output_dir}/cavitation_comparison_{type1}_vs_{type2}_z_slice_{medium_type}_{focal_depth_mm}mm.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"Saved Z-slice cavitation comparison plot to {output_file}")

    return difference, max_diff, mean_diff, std_diff, relative_diff


def create_combined_cavitation_plot(data, focal_depth_mm, medium_type, output_dir,
                           roi_start=10, roi_end=None, exclude_interface=False, interface_depth=5):
    """
    Create combined plot for Cavitation Probability across all signal types
    ...
    # Get the middle indices for each dimension
    cavitation_shape = data[first_type]['cavitation_prob'].shape
    mid_x = cavitation_shape[0] // 2
    mid_y = cavitation_shape[1] // 2
    ...
    for i, signal_type in enumerate(signal_types):
        cavitation = data[signal_type]['cavitation_prob']

        # Plot X-slice (YZ plane at center of x)
        im_x = axes_x[i].imshow(cavitation[mid_x, :, :].T,
                              extent=[y.min(), y.max(), z.min(), z.max()],
                              origin='lower', cmap='viridis', vmin=0, vmax=1.0)
        axes_x[i].set_title(f'{signal_type.replace("_", " ").title()}')
        axes_x[i].set_xlabel('Y (mm)')
        axes_x[i].set_ylabel('Z (mm)')
        plt.colorbar(im_x, ax=axes_x[i], label='Cavitation Probability')
        ...
        im_y = axes_y[i].imshow(cavitation[:, mid_y, :].T,
                              extent=[x.min(), x.max(), z.min(), z.max()],
                              origin='lower', cmap='viridis', vmin=0, vmax=1.0)
        axes_y[i].set_title(f'{signal_type.replace("_", " ").title()}')
        axes_y[i].set_xlabel('X (mm)')
        axes_y[i].set_ylabel('Z (mm)')
        plt.colorbar(im_y, ax=axes_y[i], label='Cavitation Probability')
        ...
        im_z = axes_z[i].imshow(cavitation[:, :, focal_z_idx],
                              extent=[x.min(), x.max(), y.min(), y.max()],
                              origin='lower', cmap='viridis', vmin=0, vmax=1.0)
        axes_z[i].set_title(f'{signal_type.replace("_", " ").title()}')
        axes_z[i].set_xlabel('X (mm)')
        axes_z[i].set_ylabel('Y (mm)')
        plt.colorbar(im_z, ax=axes_z[i], label='Cavitation Probability')
    ...
    # Add titles
    fig_x.suptitle(f'Cavitation Probability X-Slice Comparison ({medium_type.capitalize()}, {focal_depth_mm} mm focal depth)\n{roi_text}', fontsize=16)
    fig_y.suptitle(f'Cavitation Probability Y-Slice Comparison ({medium_type.capitalize()}, {focal_depth_mm} mm focal depth)\n{roi_text}', fontsize=16)
    fig_z.suptitle(f'Cavitation Probability Z-Slice Comparison at Focal Plane ({medium_type.capitalize()}, {focal_depth_mm} mm focal depth)', fontsize=16)
    ...
    # Save figures
    output_file_x = f'{output_dir}/cavitation_combined_x_slice_{medium_type}_{focal_depth_mm}mm.png'
    output_file_y = f'{output_dir}/cavitation_combined_y_slice_{medium_type}_{focal_depth_mm}mm.png'
    output_file_z = f'{output_dir}/cavitation_combined_z_slice_{medium_type}_{focal_depth_mm}mm.png'
    ...
    print(f"Saved combined cavitation X-slice plot to {output_file_x}")
    print(f"Saved combined cavitation Y-slice plot to {output_file_y}")
    print(f"Saved combined cavitation Z-slice plot to {output_file_z}")


if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description='Compare Cavitation Probability across different signal types')
    ...
    compare_cavitation_probability(args.focal_depth, args.medium,
